...
class JIIF(nn.Module):
    ...
    def query(self, feat, coord, hr_guide, lr_guide, image):
        ...
        B, N, _ = coord.shape    # low-res coord [30720, 2]

        # LR centers' coords
        feat_coord = make_coord((h, w), flatten=False).to(feat.device).permute(2, 0, 1).unsqueeze(0).expand(b, 2, h, w)
        # print('F.grid_sample: ', (F.grid_sample(hr_guide, coord.flip(-1).unsqueeze(1), mode='nearest', align_corners=False)).shape)  # [1, 128, 1, 30720]
        # print('hr_guide: ', hr_guide.shape, 'coord: ', coord.shape)  # hr_guide:  torch.Size([1, 128, 480, 640]) coord:  torch.Size([1, 30720, 2])
        ...
        for vx in [-1, 1]:
            for vy in [-1, 1]:
                ...
                inp = torch.cat([q_feat, q_guide, rel_coord], dim=-1)   # features of depth, HR_rgb, (HR-LR)_rgb, coord_diff

                pred = self.imnet(inp.view(B * N, -1)).view(B, N, -1)  # [B, N, 2] one for kernel, one for weight for kernel
                preds.append(pred)
